...
knapsack :: knapsack(std :: vector <int> weights, std :: vector <int> profits, int C, int population, int maxIter) : att_C(C), att_weight(weights), att_profits(profits), att_best(-1, {0}), att_iterated(maxIter), att_population(population) {
...
void knapsack :: initialize() {
	for (int i = 0 ; i < att_population ; ++i) {
...
std :: vector <population> knapsack :: crossover(population leftHandSign, population rightHandSign) {
	unsigned int threshold((std :: rand() % (leftHandSign.parent.size() - 2)) + 1);
